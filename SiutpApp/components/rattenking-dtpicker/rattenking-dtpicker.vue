...
<script>
	...
	export default {
		...
		watch: {
			...
			// curValue(val) {
			// 	this.curValue = val;
			// 	this.$emit('change', val, this.type);
			// },
			times(val){
				this.times = val;
			},
			...
			reset(val) {
				console.log('this.reset', this.reset, val)
				if (val) this.curValue = this.value;
			}
		},
		created() {
			this.initData();
		...
		methods: {
			initData(){
				// 初始化默认时间和是否禁止
					this.curValue = this.default || this.value;
					// this.curValue = this.value;
				this.curDisabled = this.disabled;
				this.$emit('change', this.curValue, this.type, true);
			},
			judgeStartEndTime(){
				// 判断开始和结束时间大小
