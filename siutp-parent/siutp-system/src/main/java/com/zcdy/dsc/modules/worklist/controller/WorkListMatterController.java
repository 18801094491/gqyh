...
@Api(tags="工单任务")
@RestController
@RequestMapping("/worklist/workListMatter")
public class WorkListMatterController extends BaseController<WorkListMatter, WorkListMatterService> {
	@Autowired
    ...
    @AutoLog(value = "向工单中添加任务时的列表，无分页")
    @ApiOperation(value="向工单中添加任务时的列表，无分页", notes="向工单中添加任务时的列表，无分页")
    @GetMapping(value = "/addlist")
    public ResultData<?> addList(WorkListMatterParam workListMatterParam,
                                       HttpServletRequest req) {
        WorkListMatter searchParam = new WorkListMatter(workListMatterParam);
        searchParam.setQueryStatusCode(WorkListConstant.WORK_LIST_MATTER_STATUS);//状态数据字典
        searchParam.setQueryTypeCode(WorkListConstant.WORK_LIST_MATTER_TYPE);//类型数据字典
        searchParam.setQueryMatterLevelCode(WorkListConstant.WORK_LIST_MATTER__MATTER_LEVEL_CODE);
        searchParam.setQueryMatterTypeCode(WorkListConstant.WORK_LIST_MATTER__MATTER_TYPE_CODE);
        searchParam.setStatus(WorkListConstant.WORK_LIST_MATTER_STATUS_DEFAULT);//只看“已创建”
        searchParam.setType(WorkListConstant.WORK_LIST_TYPE_MATTER);//只看“问题”类型
        searchParam.setDelFlag(DelFlagConstant.NORMAL);//未删除
        List<WorkListMatter> list = workListMatterService.getAddList(searchParam);
