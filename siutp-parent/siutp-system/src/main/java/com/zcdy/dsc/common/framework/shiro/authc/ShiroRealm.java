...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        if (token == null) {
            if (log.isInfoEnabled()) {
                log.info("————————身份认证失败——————————IP地址:  " + IPUtil.getIpAddr(SpringContextUtils.getHttpServletRequest()));
            }
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        String userTokenPrefix = CommonConstant.PREFIX_USER_TOKEN + userName;
        ServletRequestAttributes servletRequestAttributes = (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
        if(servletRequestAttributes != null){
            HttpServletRequest request = servletRequestAttributes.getRequest();
            String header = request.getHeader("X-request-platform");
            if (StringUtils.isNotBlank(header) && LoginTypeConstant.APP.equals(header)) {
                userTokenPrefix = CommonConstant.PREFIX_USER_TOKEN + userName + "_"+LoginTypeConstant.APP;
            }
        }
        String cacheToken = String.valueOf(redisService.get(userTokenPrefix + ":" + token));
        if (ConvertUtils.isNotEmpty(cacheToken)) {
            // 校验token有效性
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
                String newAuthorization = JwtUtil.sign(userName, passWord);
                // 设置超时时间
                redisService.set(userTokenPrefix + ":" + token, newAuthorization);
                redisService.expire(userTokenPrefix  + ":" + token, JwtUtil.EXPIRE_TIME * 2 / 1000);
            }
            return true;
        }
        return false;
