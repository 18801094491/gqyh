...
public class SysRoleController {
    ...
    private final String SYSTEM_ADMIN_ROLECODE = "admin";
	 ...
	 * @return
	 */
	@ApiOperation(value = "角色表-启停用 true为启用，false为停用", notes = "角色表-启停用")
	@GetMapping(value = "/startOrStop")
	public Result<?> startOrStop(String id,Boolean status) {
		sysRoleService.startOrStop(id,status);
	...
	public Result<List<SysRole>> queryall() {
		...
		boolean hasAdmin = SecurityUtils.getSubject().hasRole(SYSTEM_ADMIN_ROLECODE);
		List<SysRole> list = null;
		if(hasAdmin){
		    list = sysRoleService.list();
		}else{
		    QueryWrapper<SysRole> queryWrapper = new QueryWrapper<>();
		    queryWrapper.lambda().ne(SysRole::getRoleCode, SYSTEM_ADMIN_ROLECODE);
		    list = sysRoleService.list(queryWrapper);
		}
	...
	public ModelAndView exportXls(SysRole sysRole,HttpServletRequest request) {
		...
		mv.addObject(NormalExcelConstants.PARAMS,new ExportParams("角色列表数据",secondTitle,"导出信息"));
