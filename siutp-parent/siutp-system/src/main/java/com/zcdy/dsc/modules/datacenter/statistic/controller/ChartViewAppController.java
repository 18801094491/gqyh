...
@Api(tags = "数据看板")
@RestController
@RequestMapping("/statistic/chartView/app")
public class ChartViewAppController {
    @Autowired
    ...
    public Result<List<Chart>> getAllChartInfo() {
        ...
        List<Chart> chartList = this.chartService.list(queryWrapper);
        result.setResult(chartList);
        result.success("查询成功");
        return result;
    }
     ...
     * @return
     */
    @ApiOperation(value = "根据图表id获取图表数据", notes = "根据图表id获取图表数据")
    @PostMapping("/{id}")
    @ApiVersion(group = VersionConstant.VERSION_APP)
    public Result<ChartInfo> show(@ApiParam(name = "id", value = "图表id", required = true) @PathVariable String id, @RequestBody TimeWidthParam param) {
        Result<ChartInfo> result = new Result<>();
...
//			data.setName(serial.getEquipmentSection()+"["+serial.getEquipmentLocation()"]");
     ...
     *
     * @param leftTime   开始时间
     * @param rightTime  结束时间
     * @param sqlBuilder sql字符串对象
     * @param varNames   变量列表
     * @param size       变量数量
     * @return
     */
