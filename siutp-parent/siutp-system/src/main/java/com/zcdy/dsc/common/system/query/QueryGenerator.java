...
public class QueryGenerator {
	...
	private static QueryRuleEnum convert2Rule(Object value) {
		...
		if (rule == null && val.contains(COMMA)) {
			// in 查询这里应该有个bug  如果一字段本身就是多选 此时用in查询 未必能查询出来
	...
	private static Date getDateQueryByRule(String value,QueryRuleEnum rule) throws ParseException {
		...
		if(value.length()==10) {
			if(rule==QueryRuleEnum.GE) {
				...
				//比较小于
				date = formatter.parse(value + " 23:59:59");
			}
		}
		if(date==null) {
	...
	public static Map<String, SysPermissionDataRule> getRuleMap() {
		...
		Map<String, SysPermissionDataRule> ruleMap = new HashMap<String, SysPermissionDataRule>(10);
