package com.zcdy.dsc.modules.message.job;
...
public class SendMsgJob implements Job {
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		log.info(String.format(" 发送消息任务 SendMsgJob !  时间:" + System.currentTimeMillis()));
		...
		for (SysMessage sysMessage : sysMessages) {
			...
			try {
				...
				} else if (sysMessage.getEsType().equals(SendMsgTypeEnum.SYS.getType())) {
					sendMsgHandle = (ISendMsgHandle) Class.forName(SendMsgTypeEnum.SYS.getImplClass()).newInstance();
				}
			} catch (Exception e) {
