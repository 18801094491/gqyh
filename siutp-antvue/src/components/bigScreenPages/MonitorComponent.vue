...
<script>
    ...
    export default {
        ...
        props: ['id','monitorIndex', 'bodyScale'],
        data(){
            return{
                swfWidth: "",
                swfHeight: "",
        ...
        created() {
            this.swfWidth = this.id === 'monitor1' ? this.bodyScale * '500' : this.bodyScale * '241'
            this.swfHeight = this.id === 'monitor1' ? this.bodyScale * '316' : this.bodyScale * '146'
        },
        mounted() {
            this.getMonitorPoint();
            // this.swfHeight = document.getElementById("monitorPage").offsetHeight / 2;
            // this.swfWidth = document.getElementById("monitorPage").offsetWidth / 2;
            //页面加载时创建播放实例初始化
            this.initPlugin();

            // 监听resize事件，使插件窗口尺寸跟随DIV窗口变化
            var bodyScale = document.querySelector('body').offsetWidth/3200;
            $(window).resize(() => {
                bodyScale = document.querySelector('body').offsetWidth/3200;
                document.querySelector('.bigScreenPages').style.transform = `scale(${bodyScale})`;
                this.swfWidth = this.id === 'monitor1' ? bodyScale * '500' : bodyScale * '241';
                this.swfHeight = this.id === 'monitor1' ? bodyScale * '316' : bodyScale * '146';
        ...
        methods: {
            ...
            getMonitorPoint(){
                let _this = this;
                let index = this.monitorIndex;
                axios.get('rdp/monitor-point/confs')
                    .then(res => {
                        if (res.code * 1 == 200) {
                            console.log(res.data)
                            _this.records = res.data.records;
                            _this.firstScreen =  res.data.records[index],
                            _this.hkMonitorKey = res.data.records[index].hkMonitorKey,
                            _this.getArtemisConfig(res.data.records[index].hkMonitorKey)
