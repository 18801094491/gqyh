...
// 客户用水量统计-折线图
const queryAllDateWaterFlow = (data, _this) => {
    axios.get('/equipment/meterFlow/queryAllDateWaterFlow', {
        params: data
    }).then(res => {
        if (res.code * 1 == 200) {
            let list = res.result;
            _this.dataChart(list);
        } else {
    ...
    methods: {
        hdLLBSearchQuery() {
            let data = {
                ...
                startDate: this.hdLLBTime.length ? this.moment(this.hdLLBTime[0]).format('YYYY-MM-DD') : '',
                endDate: this.hdLLBTime.length ? this.moment(this.hdLLBTime[1]).format('YYYY-MM-DD') : ''
            }
            this.loadData(1, this.treeInfo.id);
            this.getEchart(data)
        },
        getEchart(data) {
            queryAllDateWaterFlow(data, this)
        },
        dataChart(data) {
...
//waterQuantityAnalysis/right.vue 页面组件混入了 aterQuantityAnalysis/right.js