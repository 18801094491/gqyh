import {
    ...
    getEquipmentList,
    getPlanDate
} from '@/api/operationCenter-t/equipmentAccountMaintenance.js'
import { colAuthFilter } from "@/utils/authFilter"
import JTreeLabelSelect from "@/views/operationCenter/equipmentAccountMaintenance/JTreeLabelSelect"
...
export default {
    ...
    data() {
        return {
            ...
            hkMonitorCode: '', //监控标识
            hkMonitorKey: '', //监控平台
            hkMonitorKeyData: [], //监控平台数据
            isShowkMonitorCode: false, //是否显示监控标识
            supplier: '', //供应商
            state: '', //状态,
            supplierClassificationList: [], //供应商
            supplierClassificationList2: [],
            equipmentCategoryList: [], //资产类别
            ...
            managerList2:[], // 养护收件人下拉
            rules:{
            	days:[
            		{ required: true, message: '养护到期提醒时间不能为空', trigger: 'blur' },
            	]
            },
            // 分派工单相关
            workerVisible:false,
...
//  	this.initDictConfig();
    ...
    mounted() {
        ...
        // 获取用户
        getUserData({name:''},this);
        getTeamDutyTeamList(this); // 获取班组人员下拉
        getEquipmentList('/opt/upkeep/plan/unDispatchEquipment',this); // 获取设备信息下拉
    },
    methods: {
    	//获取监控平台
        getMonitorPlatform () {
            (function (_this) {
                ...
                axios.get(url).then( (res) => {
                    if (res.data.code * 1 == 200) {
                        if (res.data.list) {
                            _this.hkMonitorKeyData = res.data.list;
                            _this.hkMonitorKey = res.data.list[0].key;
                        }
                    } else {
    	...
    	setDateOk(){
    		this.$refs.ruleForm.validate(valid => {
                if (valid) {
                	let data = {
		    			days:this.setForm.days,
		    			usersId:this.setForm.usersId?this.setForm.usersId.join(','):''
		    		}
		    		setPlanDate(data,this)
                }else{
                	return false;
                }
          	})    		
...
//          this.stateList[index].imgUrl='';
        ...
        handleAdd(type, data) {
            ...
            if (type == 'add') {
                ...
                if(data.equipmentTypeName == '监控'){
                    this.isShowkMonitorCode = true;
                    this.getMonitorPlatform(); //获取监控平台
                }
        ...
        handleChange(data) {
            ...
            this.personResponsible = data.personResponsible?data.personResponsible.split(','):[];
            //console.log(this.personResponsible,'this.personResponsible')
                ...
                if (!err) {
                    ...
                    if (type == 'add') {
                        data = {
                            ...
                            personResponsible: this.personResponsible?this.personResponsible.join(','):'',
                        ...
                        data = {
                            ...
                            hkMonitorCode: $.trim(this.hkMonitorCode),
                            hkMonitorKey: this.hkMonitorKey,
                        }
...
//      this.jumpPagNum = '';
        ...
        labelSubmit() {
            let data = {
                labelId: this.labelId, //地区id
                equipmentId: this.model.equipmentId, //设备id
            }
            labelBind(data, this)
        },
        handleCancellabel() {
        ...
        labelUnbind(record) {
            let data = {
                labelId: record.labelId, //地区id
                equipmentId: record.id, //设备id
            }
            labelUnbind(data, this)
        },
        labelDetail(record) {
            this.model.equipmentId = record.id
            this.model.labelId = record.labelId
            this.labelVisible = true;
            this.$nextTick(() => {
                setTimeout(()=>{
                    this.labelForm.setFieldsValue(pick(this.model,'labelId'))
                })
            })
        },
        updatePlan(record){ // 修改计划日期
        	this.optId = record.id;
        	this.planVisible = true;
        	this.planDate = record.planDate?record.planDate:''
        },
        updatePlanOk(){
        	if(!this.planDate){
        	...
        	updatePlan({optId:this.optId,planDate:this.moment(this.planDate).format('YYYY-MM-DD')},this)
        },
        updatePlanCancel(){
        ...
        assignWoker(record){ // 分派工单
        	...
        	this.equipmentId.push(record.id);
        ...
        handleOk(){
        	let data = {
        		...
        		jobType:'0', // 0-养护，1-维修，2-检修
        		workTeamId:this.workTeamId,
        		id:this.assinId
        	}
        	updateAssignWoker(data,this,'/opt/upkeep/plan/dispatch','up')
        },
        handleCancel(){
...
//equipmentAccountMaintenance/index.vue 页面混入了equipmentAccountMaintenance/index.js