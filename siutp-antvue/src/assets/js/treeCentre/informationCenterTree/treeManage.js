...
export default {
    ...
    data() {
        return {
            ...
            parentUrl: '/centre/inf/equ/getAll/',
            expandedRowKeys: [],
...
//  	this.initDictConfig();
    ...
    methods: {
        ...
        handleOk() {
            ...
            this.$nextTick(() => {
                if(!parentId){
                    _this.$message.info("请选择父节点");
                    return false
                }
                httpAction('/centre/inf/equ/treeobj/bind/', data, 'post')
                    .then(res => {
                        let result = res.data;
                        if (res.code * 1 == 200) {
        ...
        handleDeletes (id) {
            ...
            let parentId = '';
            let data = {dataId: id};
            httpAction('/centre/inf/equ/treeobj/unbind/', data, 'post')
                .then(res => {
                    let result = res.data;
                    if (res.code * 1 == 200) {
                        _this.$message.success(res.message);
