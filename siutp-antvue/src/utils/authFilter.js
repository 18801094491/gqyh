...
export function colAuthFilter(columns,pre) {
	console.log(columns,pre,'columns,precolumns,precolumns,precolumns,pre')
...
//console.log(cols,'1111111111cols')
  return cols
}

function hasColoum(item,authList){
//	console.log(item,authList,'item,authListitem,authListitem,authListitem,authListitem,authList')
...
function getNoAuthCols(pre){
  ...
  //let authList = Vue.ls.get(USER_AUTH);
  let authList = JSON.parse(sessionStorage.getItem(USER_AUTH) || "[]");
  console.log(authList,'authListauthListauthListauthList')
  for (var auth of authList) {
    //显示策略，有效状态
  ...
  console.log(cols,'colscolscolscolscols')
  return cols;
}
